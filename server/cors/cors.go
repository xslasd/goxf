...
func New(options Options) *Cors {
	c := &Cors{
		...
		allowedWOrigins:     make([]wildcard, 0),
	...
	if options.AllowOriginFunc != nil {
		c.allowOriginFunc = options.AllowOriginFunc
	}
...
func DefaultOptions() Options {
	return Options{
		...
		AllowedHeaders:   []string{"Content-Type", "Authorization", "Content-Length", "X-CSRF-Token", "session", "x-requested-with", "Response-Type"},
...
func (c *Cors) handlePreflight(w http.ResponseWriter, r *http.Request) {
	...
	c.logf("  Preflight api headers: %v", headers)
}
...
func (c *Cors) handleActualRequest(w http.ResponseWriter, r *http.Request) {
	...
	c.logf("  Actual api added headers: %v", headers)
}
